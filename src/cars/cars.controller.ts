...
export class CarsController {
  ...
  async getModels(@Param('brandId') brandId: string) {
    // Извлекает параметр brand из URL преоброзует его число
  ...
  async addCar(@Request() req, @Body() dto: CreateUserCarDto) {
    // Извлекает тело запроса и валидирует его по DTO
  ...
  async updateCar(
    ...
    return this.carsService.updateUserCar(id, req.user.sub, dto);
  }
